...
int main(int argc,char* argv[]){
    ...
    while(cnt <= 15){
        ...
        //假如我们的客户端一次发送多个相同的请求
        // cout << cnt << "次发的数据:" << package << endl; 
        // write(sock.getSockfd(),package.c_str(),package.size());
        // cout << cnt << "次发的数据:" << package << endl; 
        // write(sock.getSockfd(),package.c_str(),package.size());
        // cout << cnt << "次发的数据:" << package << endl; 
        // write(sock.getSockfd(),package.c_str(),package.size());
        // cout << cnt << "次发的数据:" << package << endl; 
        // write(sock.getSockfd(),package.c_str(),package.size());
        //那客户端接收的时候 也应该像服务器那样再写一个循环去处理一次接收多个response的情况
        //处理完了 在进行第二次的发送 但这里就不赘述了 因为我们这次编码的重心是在服务端
        //主要是理解 对任意报文接收的处理方式就行
